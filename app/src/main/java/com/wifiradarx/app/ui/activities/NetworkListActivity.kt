...
// FIX: Now observes combinedScans which shows live results when DB is empty.
class NetworkListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // FIX: Use combinedScans (live + DB merged) instead of allDbScans alone.
        lifecycleScope.launch {
            vm.combinedScans.collectLatest { scans ->
                ...
                tvCount?.text = "${filtered.size} networks"
                // Show source indicator
                val isLive = scans.any { it.sessionId == "live" }
                if (isLive) tvCount?.append(" (live)")
            }
        }
    }
}
