...
class SecurityAuditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // FIX: Audit from DB records when available…
        lifecycleScope.launch {
            mainVm.allScans.collectLatest { scans ->
                if (scans.isNotEmpty()) vm.audit(scans)
            }
        }
        // FIX: …and fall back to live scan results immediately so the screen isn't blank.
        lifecycleScope.launch {
            mainVm.scanResults.collectLatest { live ->
                vm.auditLive(live)
            }
        }
...
class AuditAdapter :
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.tvBadges.text = audit.badges.joinToString(" · ").ifBlank { "—" }
