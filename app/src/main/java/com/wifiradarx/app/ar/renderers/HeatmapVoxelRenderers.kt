...
class HeatmapMeshRenderer {
    ...
    fun updateMesh(grid: Array<FloatArray>, minX: Float, minZ: Float, y: Float, resolution: Float) {
        ...
        vertexBuffer = ByteBuffer.allocateDirect(vertices.size * 4).run { order(ByteOrder.nativeOrder()); asFloatBuffer().apply { put(vertices); position(0) } }
        rssiBuffer = ByteBuffer.allocateDirect(rssis.size * 4).run { order(ByteOrder.nativeOrder()); asFloatBuffer().apply { put(rssis); position(0) } }
    }
    ...
    fun draw(viewMatrix: FloatArray, projectionMatrix: FloatArray) {
        ...
        GLES20.glVertexAttribPointer(rssiHandle, 1, GLES20.GL_FLOAT, false, 0, rssiBuffer)
